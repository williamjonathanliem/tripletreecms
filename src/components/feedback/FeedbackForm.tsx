  ...
  return (
    <div className="flex items-center gap-3 mb-2">
      ...
      <span className="text-sm font-semibold text-gray-800">
        {label}
        {optional && <span className="ml-1.5 text-xs font-normal text-gray-400">({t.common.optional})</span>}
      </span>
    </div>
  )
}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="px-6 py-5 border-b border-gray-50">
        <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">{t.feedback.new_entry}</p>
        <h2 className="text-base font-bold text-gray-900 mt-0.5">{t.feedback.log_class}</h2>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm font-semibold text-gray-800">{t.feedback.which_class}</span>
          </div>
          <Select value={watchedClassId} onValueChange={(v: string | null) => { if (v) setValue('class_id', v, { shouldValidate: true }) }}>
            <SelectTrigger className="bg-gray-50 border-gray-200 rounded-xl h-11 focus:border-[#1E8449] focus:ring-0 focus:outline-none">
              {selectedClass
                ...
                : <span className="text-sm text-gray-400">{t.feedback.select_class}</span>}
            </SelectTrigger>
            <SelectContent>
              {classes.length === 0
                ? <div className="py-4 text-center text-sm text-gray-400">{t.feedback.no_classes}</div>
                : classes.map(c => (
        ...
        <div>
          <Step n={1} label={t.feedback.step1_label} />
          <textarea
            {...register('how_was_class')}
            rows={3}
            placeholder={t.feedback.step1_placeholder}
        ...
        <div>
          <Step n={2} label={t.feedback.step2_label} />
          <textarea
            {...register('topics_covered')}
            rows={2}
            placeholder={t.feedback.step2_placeholder}
            className={textareaBase}
          />
        ...
        <div>
          <Step n={3} label={t.feedback.step3_label} optional />
          <textarea
            {...register('other_comments')}
            rows={2}
            placeholder={t.feedback.step3_placeholder}
            className={textareaBase}
          />
        </div>
